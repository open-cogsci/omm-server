...
class ResultsViewEnhancedSchema extends Schema {
  up () {
    ...
    WHERE job_states.data IS NOT NULL
    `)
  }
  ...
  down () {
    ...
    CREATE VIEW results_view AS
    SELECT jobs.id AS job_id, jobs.position, jobs.study_id,
        job_states.data, job_states.updated_at AS last_recording,
    ...
    WHERE job_states.data IS NOT NULL
    GROUP BY job_id, job_states.data, status, participant_id, participants.identifier, participants.meta`)
  }
}
